<template>
  <admin-panel-layout v-if="!categories.data.length" title="Kategorie sprzętu">BRAK DANYCH</admin-panel-layout>
			...
			<form @submit.prevent="store, update">
                ...
                <div class="mt-6">
                    <select v-model=form.parentCategoryId>
                        ...
                        <option v-for="row in categories.data" :key=row :value="row.id"> {{ row.name }} </option>
                    </select>
