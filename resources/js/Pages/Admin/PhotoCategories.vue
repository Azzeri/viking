...
export default defineComponent({
	...
	setup(props) {
		...
		const createSubcategory = _ => {
			...
			form.photo_category_id = selectedCategory.value.id
        ...
        const store = (is_subcategory) => { 
			form.post(route('admin.photo_categories.store'), {
				onSuccess: () => {
					reset()
					if (is_subcategory) {
						form.photo_category_id = selectedCategory.value.id
		...
		const update = (category) => { 
			form.put(route('admin.photo_categories.update', category.id), {
				onSuccess: () => {
					resetModes(true, true, true)
					selectedCategory.value = props.categories.data.find(element => element.id == selectedCategory.value.id)
				} 
        ...
        const deleteRow = (id, is_subcategory) => {
            if (!confirm('Na pewno? Wszystkie zdjęcia należące do kategorii również zostaną usunięte!')) return;
			resetModes(true, true, true)
            Inertia.delete(route('admin.photo_categories.destroy', id), {
				onSuccess: () => {
					if (is_subcategory)
						selectedCategory.value = props.categories.data.find(element => element.id == selectedCategory.value.id)
					else close()
