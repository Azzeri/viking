...
<admin-panel-layout title="Sklep">
	...
	<template v-if="(!items.data.length || !categories.length) && filters.search == null">
		...
		<div class="flex flex-col gap-4 justify-center items-center">
			<Link :href="route('admin.store_categories.index')" class="btn btn-wide btn-secondary">
				Kategorie
			</Link>
		...
		<DataTable :columns=columns :data=items :filters=filters sortRoute="admin.store_items.index">
			<template #buttons>
				...
				<div class="flex justify-center space-x-2">
					<Link :href="route('admin.store_categories.index')" class="btn btn-secondary sm:btn-sm">
						Kategorie
					</Link>
					<Link :href="route('admin.store_requests.index')" class="btn btn-secondary sm:btn-sm">
						Zamówienia
					</Link>
				</div>
			</template>
					...
					<div class="flex mt-4">
						...
						<div class="flex flex-col">
							...
							<div class="flex gap-2 ml-1 mt-2">
								<span class="font-semibold">{{ `${selectedItem.price}zł` }}</span>
					...
					<form @submit.prevent="update">
						...
						<div class="flex my-4 space-x-2 w-full">
							...
							<div class="flex flex-col mt-1 gap-2 justify-center w-full">
								<select v-model=form.store_category_id class="select select-bordered select-primary select-sm text-sm w-full">
...
export default defineComponent({
	...
	setup(props) {
		...
		// Update item
		const update = _ => { 
			form.post(route(`admin.store_items.update`, { store_item:selectedItem.value.id, _method:'put' }), {
				onSuccess: () => {
					reset()
